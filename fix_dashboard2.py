...
document.getElementById("loginForm").addEventListener("submit", function(e) {
    ...
    validateApiKey(apiKey).then(function(valid) {
        if (valid) {
...
function loadStats() {
    fetch("/api/v1/incidents", {headers: {"X-API-Key": apiKey}})
        ...
        .then(function(d) {
            var i = d.data || [];
            var c = i.filter(function(x) { return x.state === "CLOSED"; }).length;
            var r = i.length > 0 ? Math.round(c / i.length * 100) : 0;
            var h = Date.now() - 3600000;
            var n = i.filter(function(x) { return new Date(x.created_at).getTime() > h; }).length;
            var t = i.reduce(function(s, x) { return s + (x.tokens_used || 0); }, 0);
            document.getElementById("eventsHour").textContent = n;
            document.getElementById("automationRate").textContent = r + "%";
            document.getElementById("tokensToday").textContent = (t / 1000).toFixed(1) + "k";
            document.getElementById("tokensCost").textContent = "~$" + (t / 1000 * 0.008).toFixed(2) + " est.";
        })
        .catch(function(e) { console.error(e); });
}

function loadIncidents() {
    var s = document.getElementById("stateFilter").value;
    var u = s ? "/api/v1/incidents?state=" + s : "/api/v1/incidents";
    fetch(u, {headers: {"X-API-Key": apiKey}})
        .then(function(r) { return r.json(); })
        .then(function(d) {
            var i = d.data || [];
            document.getElementById("activeIncidents").textContent = i.filter(function(x) { return ["CLOSED", "ERROR"].indexOf(x.state) === -1; }).length;
            document.getElementById("incidentsTable").innerHTML = i.map(function(x) {
                return "<tr class='incident-row border-b border-gray-800/50 cursor-pointer' onclick=\"viewIncident('" + x.id + "')\"><td class='p-3 mono text-xs text-gray-300'>" + x.id.substring(0, 8) + "</td><td class='p-3'><span class='badge px-2 py-1 rounded " + getStateColor(x.state) + "'>" + x.state + "</span></td><td class='p-3'><span class='badge px-2 py-1 rounded " + getSeverityColor(x.severity) + "'>" + (x.severity || "-") + "</span></td><td class='p-3'><div class='flex items-center'><span class='mono text-xs mr-2'>" + (x.confidence * 100).toFixed(0) + "%</span><div class='w-12 h-1 bg-gray-800 rounded overflow-hidden'><div class='h-full " + getConfidenceColor(x.confidence) + "' style='width:" + (x.confidence * 100) + "%'></div></div></div></td><td class='p-3'><div class='mono text-xs'>" + (x.tokens_used || 0).toLocaleString() + "</div><div class='token-bar mt-1'><div class='token-bar-fill' style='width:" + Math.min((x.tokens_used || 0) / 50000 * 100, 100) + "%'></div></div></td><td class='p-3 mono text-xs text-gray-400'>" + getAge(x.created_at) + "</td><td class='p-3'><button class='text-amber-500 hover:text-amber-400 text-xs'>View</button></td></tr>";
            }).join("") || "<tr><td colspan='7' class='p-8 text-center text-gray-600'>No incidents</td></tr>";
        })
        .catch(function(e) { console.error(e); });
}
...
function loadRecentFindings() {
    fetch("/api/v1/incidents/findings/recent?limit=5", {headers: {"X-API-Key": apiKey}})
        .then(function(r) { return r.json(); })
        .then(function(f) {
            document.getElementById("recentFindings").innerHTML = f.map(function(x) {
                return "<div class='findings-panel p-2 rounded text-xs'><div class='flex justify-between mb-1'><span class='text-amber-400 font-medium'>" + x.agent + "</span><span class='text-gray-500'>" + (x.tokens_consumed || 0) + " tok</span></div><div class='text-gray-400 truncate'>" + (x.reasoning ? x.reasoning.substring(0, 80) : "No reasoning") + "...</div></div>";
            }).join("") || "<div class='text-gray-600'>No findings</div>";
        })
...
function loadRequiresAttention() {
    fetch("/api/v1/incidents", {headers: {"X-API-Key": apiKey}})
        .then(function(r) { return r.json(); })
        .then(function(d) {
            var n = (d.data || []).filter(function(x) {
                return x.state === "ESCALATED" || x.state === "PROPOSAL" || x.state === "ERROR" || (x.state !== "CLOSED" && x.confidence >= 0.8 && x.severity === "CRITICAL");
            });
            var c = document.getElementById("requiresAttention");
            if (n.length === 0) {
                c.innerHTML = "<div class='text-green-400 text-xs'>OK - No threats need attention</div>";
                return;
            }
            c.innerHTML = n.map(function(x) {
                return "<div class='p-2 rounded cursor-pointer hover:bg-red-500/10 border border-red-500/20' onclick=\"viewIncident('" + x.id + "')\"><div class='flex justify-between items-center mb-1'><span class='mono text-xs'>" + x.id.substring(0, 8) + "</span><span class='badge px-1 py-0.5 rounded " + getStateColor(x.state) + "'>" + x.state + "</span></div><div class='flex justify-between text-xs'><span class='" + getSeverityColor(x.severity) + " px-1 rounded'>" + x.severity + "</span><span class='text-gray-400'>" + (x.confidence * 100).toFixed(0) + "% conf</span></div><div class='text-xs text-gray-500 mt-1'>" + getAttentionReason(x) + "</div></div>";
            }).join("");
        })
        .catch(function() { document.getElementById("requiresAttention").innerHTML = "<div class='text-gray-600 text-xs'>Unable to load</div>"; });
}

function getAttentionReason(i) {
    if (i.state === "ESCALATED") return "!! Escalated - needs review";
    if (i.state === "PROPOSAL") return ">> Action pending approval";
    if (i.state === "ERROR") return "XX Processing error";
    if (i.confidence >= 0.8 && i.severity === "CRITICAL") return "** High confidence threat";
    return "Requires review";
}

function loadAgentActivity() {
    var agents = ["intel", "analyst", "responder", "oversight", "router"];
    document.getElementById("agentActivity").innerHTML = agents.map(function(a) {
        return "<div class='flex justify-between items-center'><span class='text-gray-400'>" + a + "</span><span class='text-green-400 text-xs'>* Ready</span></div>";
    }).join("");
}

function connectWebSocket() {
    var p = location.protocol === "https:" ? "wss:" : "ws:";
    ws = new WebSocket(p + "//" + location.host + "/api/v1/ws/default-tenant?api_key=" + apiKey);
    ws.onopen = function() { document.getElementById("wsStatus").innerHTML = "<span class='text-green-400'>* WebSocket Live</span>"; };
    ws.onmessage = function(e) {
        var m = JSON.parse(e.data);
        if (m.type === "incident_update" || m.type === "incident_created") {
            loadIncidents();
            loadStats();
            loadRequiresAttention();
        }
    ...
    ws.onclose = function() {
        document.getElementById("wsStatus").innerHTML = "<span class='text-yellow-400'>* Reconnecting...</span>";
...
function getAge(t) {
    var d = Date.now() - new Date(t).getTime();
...
function viewIncident(id) {
    ...
    fetch("/api/v1/incidents/" + id, {headers: {"X-API-Key": apiKey}})
        .then(function(r) { return r.json(); })
        .then(function(inc) {
            fetch("/api/v1/incidents/" + id + "/findings", {headers: {"X-API-Key": apiKey}})
                .then(function(r) { return r.json(); })
                .catch(function() { return []; })
                .then(function(f) {
                    document.getElementById("modalContent").innerHTML = "<div class='grid grid-cols-2 gap-6'><div><div class='border-l-2 border-amber-500/30 pl-4 mb-6'><h4 class='text-xs text-gray-400 mb-2'>STATUS</h4><div class='flex space-x-2'><span class='badge px-2 py-1 rounded " + getStateColor(inc.state) + "'>" + inc.state + "</span><span class='badge px-2 py-1 rounded " + getSeverityColor(inc.severity) + "'>" + inc.severity + "</span></div></div><div class='border-l-2 border-amber-500/30 pl-4 mb-6'><h4 class='text-xs text-gray-400 mb-2'>METRICS</h4><div class='grid grid-cols-2 gap-4 text-sm'><div><div class='text-gray-500 text-xs'>Confidence</div><div class='mono text-lg'>" + (inc.confidence * 100).toFixed(0) + "%</div></div><div><div class='text-gray-500 text-xs'>Tokens</div><div class='mono text-lg'>" + (inc.tokens_used || 0).toLocaleString() + "</div></div><div><div class='text-gray-500 text-xs'>Created</div><div class='mono text-sm'>" + new Date(inc.created_at).toLocaleString() + "</div></div><div><div class='text-gray-500 text-xs'>Updated</div><div class='mono text-sm'>" + new Date(inc.updated_at).toLocaleString() + "</div></div></div></div><div class='border-l-2 border-amber-500/30 pl-4 mb-6'><h4 class='text-xs text-gray-400 mb-2'>SUMMARY</h4><div class='text-sm text-gray-300 bg-black/30 p-3 rounded'>" + (inc.summary || "No summary") + "</div></div></div><div><div class='border-l-2 border-amber-500/30 pl-4 mb-6'><h4 class='text-xs text-gray-400 mb-2'>AGENT FINDINGS</h4><div class='space-y-3 max-h-80 overflow-y-auto'>" + (f.length > 0 ? f.map(function(x) { return "<div class='findings-panel p-3 rounded'><div class='flex justify-between mb-2'><span class='text-amber-400 font-medium text-sm'>" + x.agent + "</span><span class='mono text-xs text-gray-500'>" + (x.tokens_consumed || 0) + " tokens</span></div><div class='text-xs text-gray-400 mb-1'>" + x.finding_type + " - " + (x.confidence * 100).toFixed(0) + "% conf</div><div class='text-sm text-gray-300'>" + (x.reasoning || "No reasoning") + "</div></div>"; }).join("") : "<div class='text-gray-600 text-sm'>No findings</div>") + "</div></div><div class='border-l-2 border-amber-500/30 pl-4'><h4 class='text-xs text-gray-400 mb-2'>RAW DATA</h4><pre class='text-xs bg-black/50 p-3 rounded overflow-x-auto max-h-40 text-gray-400 mono'>" + JSON.stringify(inc, null, 2) + "</pre></div></div></div>";
                });
        })
        .catch(function(e) { document.getElementById("modalContent").innerHTML = "<div class='text-red-400'>Failed: " + e.message + "</div>"; });
}
